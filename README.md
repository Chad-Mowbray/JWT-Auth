...
Once you get that going, if you haven't already, decide how long a token should be valid.  Then add a check to ensure that your server only accepts non-expired tokens.
 ...
 In order trust that the token was actually generated by the server, you should add a signature to your token.  You'll need to generate a public/private key pair first.
 
As usual, add validity checks to ensure that your server only accepts properly signed tokens.
